type ToptalTalentBadgeProps = {
  focusable?: boolean;
  showInfo?: boolean;
  size?: "full" | "medium" | "small" | "compact";
};

export function ToptalTalentBadge({
  focusable = true,
  showInfo = true,
  size = "full",
}: ToptalTalentBadgeProps) {
  const badge = <ToptalBadgeMark size={size} />;
  ...
  return (
    <span className="toptal-badge-info" tabIndex={focusable ? 0 : undefined}>
      {badge}
      <span className="toptal-badge-message" role="tooltip">
        <ToptalInfoCopy />
      </span>
    </span>
  );
...
export function ToptalInfoCopy() {
  return (
    <>
      <strong>What is Toptal?</strong>
      <span className="toptal-info-text">
        Toptal is a selective global talent network with a rigorous vetting process. Ali is part of its trusted
...
function ToptalBadgeMark({ size = "full" }: Pick<ToptalTalentBadgeProps, "size">) {
  if (size === "compact") {
    return (
      <span
        className="toptal-badge-shell toptal-badge-shell-compact"
        aria-label="Top 3% Talent vetted by Toptal"
        title="Top 3% Talent vetted by Toptal"
      >
        <span className="toptal-badge-compact">3%</span>
      </span>
    );
  }
  ...
  const shellClassName =
    size === "small"
      ? "toptal-badge-shell toptal-badge-shell-small"
      : size === "medium"
        ? "toptal-badge-shell toptal-badge-shell-medium"
        : "toptal-badge-shell";

  return (
    <div className={shellClassName} aria-label="Top 3% Talent vetted by Toptal">
